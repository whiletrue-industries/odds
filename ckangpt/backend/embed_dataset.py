...
def main_glob(dataset_domain, dataset_name,
              ...
              force_update=False, limit=None):
    print(f'Embedding datasets matching glob pattern {dataset_domain}/{dataset_name}')
        ...
        for idx, item in enumerate(storage.list_(prefix=f'dataset_descriptions/{domain}/')):
            ...
            if fnmatch.fnmatchcase(name.lower(), dataset_name.lower()):
                ...
                if ret:
                    ...
                    yield ret
            if idx % 100 == 99:
                print(f'Embedded {embedded} datasets so far (out of {idx + 1} datasets considered so far)') 
