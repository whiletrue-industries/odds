...
async def get_catalog_datasets(deployment_id: str, catalog_id: str, user: FireBaseUser, page: int = 1, sort: str = None, query: str = None):
    email = user['email']
    deployment = await deployment_repo.get_deployment(deployment_id)
    if not deployment:
        raise HTTPException(status_code=404, detail="Deployment not found")
    ...
    if not catalog:
        raise HTTPException(status_code=404, detail="Catalog not found")
    result = await metadata_store.getDatasets(catalog_id, page=page, sort=sort, query=query)
    simple_datasets = []
